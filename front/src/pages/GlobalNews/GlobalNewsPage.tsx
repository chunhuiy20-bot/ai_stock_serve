...
type D3Global = any;
type TopojsonGlobal = {
  feature: (topology: unknown, object: unknown) => { features: WorldFeature[] };
};

declare global {
  interface Window {
    d3?: D3Global;
    topojson?: TopojsonGlobal;
  }
}

const globalFeed: FeedData = {
...
const newsSeed: Record<string, FeedData> = {
  us: { name: 'United States', aliases: ['United States of America', 'USA', 'US'], lat: 38, lon: -97, color: '#53e0c1', summary: '美国资讯流覆盖科技、金融、政治和国际关系，通常也是全球突发新闻和资本市场波动的高密度来源。', tags: ['科技', '金融市场', '政策', 'AI'], news: [
  ...
  cn: { name: 'China', aliases: ["People''s Republic of China"], lat: 35, lon: 103, color: '#ff9b71', summary: '中国资讯通常集中在宏观经济、制造业、科技平台、跨境贸易与区域政策，覆盖面广且节奏快。', tags: ['宏观经济', '制造业', '跨境贸易', '科技'], news: [
  ...
  jp: { name: 'Japan', lat: 36, lon: 138, color: '#6bb8ff', summary: '日本新闻热点常围绕半导体、汽车、汇率、能源与地区安全议题，内容兼具产业性和国际性。', tags: ['半导体', '汽车', '汇率', '能源'], news: [
  ...
  gb: { name: 'United Kingdom', aliases: ['United Kingdom of Great Britain and Northern Ireland', 'Britain'], lat: 55, lon: -3, color: '#53e0c1', summary: '英国新闻流以金融、公共政策、科技监管与欧洲关系为主，伦敦仍是全球金融资讯核心节点之一。', tags: ['金融', '监管', '欧洲关系', '创投'], news: [
  ...
  fr: { name: 'France', lat: 46, lon: 2, color: '#ff9b71', summary: '法国资讯聚焦欧洲政治、能源转型、工业政策与文化经济，在欧盟语境下具有较强影响力。', tags: ['欧洲政治', '能源转型', '工业', '文化经济'], news: [
  ...
  br: { name: 'Brazil', lat: -14, lon: -52, color: '#6bb8ff', summary: '巴西的资讯热点多与农业、大宗商品、能源、雨林保护和拉美区域经济环境相关。', tags: ['农业', '大宗商品', '能源', '环境'], news: [
  ...
  in: { name: 'India', lat: 21, lon: 78, color: '#53e0c1', summary: '印度新闻密集分布于数字经济、制造业升级、消费增长、基础设施与区域外交。', tags: ['数字经济', '消费', '制造升级', '基建'], news: [
  ...
  za: { name: 'South Africa', lat: -30, lon: 24, color: '#ff9b71', summary: '南非资讯重点涉及矿业、能源、电力供应、区域贸易和社会治理，是观察非洲南部的重要入口。', tags: ['矿业', '电力', '区域贸易', '治理'], news: [
...
function normalizeName(name?: string | null) { return (name || '').trim().toLowerCase(); }
function seedFromName(name: string) { return Array.from(name).reduce((sum, char) => sum + char.charCodeAt(0), 0); }
function getCountryPayload(name?: string | null): FeedData {
  if (!name) return globalFeed;
  const exactKey = aliasToKey.get(normalizeName(name));
  if (exactKey) return newsSeed[exactKey];
  ...
  return {
    ...
    news: [{ source: 'Prototype Feed', time: 'just now', title: `${name || '该国家'} 已被选中`, description: '地图点击已经生效，当前显示的是占位资讯。你可以继续让我接入真实新闻接口。' }],
  };
}
function countryHasNews(name?: string) { return aliasToKey.has(normalizeName(name)); }
...
export function GlobalNewsPage() {
  ...
  const feed = getCountryPayload(selectedCountryName);
  ...
  useEffect(() => {
    ...
    async function init() {
      try {
        await ensureScript('https://cdn.jsdelivr.net/npm/d3@7/dist/d3.min.js');
        await ensureScript('https://cdn.jsdelivr.net/npm/topojson-client@3/dist/topojson-client.min.js');
        if (cancelled || !svgRef.current || !window.d3 || !window.topojson) return;
        ...
        let worldFeatures: WorldFeature[] = [];
        let rotation: [number, number, number] = [-20, -15, 0];
        let isDragging = false;
        let isPointerOverGlobe = false;
        let isLoaded = false;
        let currentSelected = selectedCountryName;

        const defs = svg.append('defs');
        const oceanGradient = defs.append('radialGradient').attr('id', 'reactOceanGradient').attr('cx', '35%').attr('cy', '30%').attr('r', '80%');
        ...
        svg.append('circle').attr('class', 'ocean-sphere').attr('cx', width / 2).attr('cy', height / 2).attr('r', 360).attr('fill', 'url(#reactOceanGradient)');
        ...
        function applyNames(nameRows: Array<{ iso_n3: string; name: string }>) {
          const nameMap = new Map(nameRows.map((row) => [row.iso_n3, row.name]));
          worldFeatures = worldFeatures.map((feature) => ({ ...feature, properties: { ...feature.properties, name: nameMap.get(String(feature.id).padStart(3, '0')) || feature.properties.name || `Country ${feature.id}` } }));
        }

        function ensureFallbackNames() {
          worldFeatures = worldFeatures.map((feature) => ({ ...feature, properties: { ...feature.properties, name: feature.properties?.name || `Country ${feature.id}` } }));
        }

        function updateMarkers() {
          const markerData = Object.values(newsSeed).map((item) => ({ ...item, coordinates: [item.lon ?? 0, item.lat ?? 0] as [number, number] }));
          const visibleMarkers = markerData.filter((item) => d3.geoDistance(item.coordinates, [-rotation[0], -rotation[1]]) < Math.PI / 2);
          const markers = markersLayer.selectAll('g.marker').data(visibleMarkers, (d: FeedData & { coordinates: [number, number] }) => d.name);
          const enter = markers.enter().append('g').attr('class', 'marker').style('color', (d: FeedData & { coordinates: [number, number] }) => d.color || '#53e0c1');
          ...
          markers.merge(enter).each(function(this: SVGGElement, d: FeedData & { coordinates: [number, number] }) {
            const point = projection(d.coordinates);
            if (!point) { d3.select(this).attr('display', 'none'); return; }
            const [x, y] = point;
            ...
            const active = d.name === currentSelected;
        ...
        function updateMap() {
          ...
          countriesLayer.selectAll('path.country').attr('d', (d: WorldFeature) => path(d as never) || '').attr('class', (d: WorldFeature) => {
            const classes = ['country'];
            if (countryHasNews(d.properties.name)) classes.push('has-news');
            if (currentSelected && d.properties.name === currentSelected) classes.push('active');
            return classes.join(' ');
          });
          updateMarkers();
        }
        ...
        function drawCountries() {
          ...
          countriesLayer.selectAll('path.country').data(worldFeatures).enter().append('path').attr('class', (d: WorldFeature) => countryHasNews(d.properties.name) ? 'country has-news' : 'country').attr('d', (d: WorldFeature) => path(d as never) || '').on('click', (_: unknown, d: WorldFeature) => {
            currentSelected = d.properties.name || null;
            setSelectedCountryName(currentSelected);
            updateMap();
          });
        }
        svg.on('mouseenter', () => { isPointerOverGlobe = true; }).on('mouseleave', () => { isPointerOverGlobe = false; });
        const drag = d3.drag()
          .on('start', () => { isDragging = true; svg.classed('is-dragging', true); })
          ...
          .on('end', () => { isDragging = false; svg.classed('is-dragging', false); });
        ...
        if (cancelled) return;
        worldFeatures = topojson.feature(topology, (topology as { objects: { countries: unknown } }).objects.countries).features;
        ensureFallbackNames();
        drawCountries();
        isLoaded = true;
        updateMap();

        d3.tsv('https://cdn.jsdelivr.net/npm/world-atlas@2/countries-110m.tsv').then((nameRows: Array<{ iso_n3: string; name: string }>) => {
          if (cancelled) return;
          applyNames(nameRows as Array<{ iso_n3: string; name: string }>);
          drawCountries();
          updateMap();
        }).catch(() => {});
  ...
  return (
    <section className="global-news-page">
      <main className="global-news-dashboard">
        <section className="global-news-globe-panel">
          <div className="global-news-panel-header"><div><p className="global-news-section-label">Interactive Globe</p></div></div>
          <div className="global-news-globe-stage">
            <div className="global-news-globe-core">
              <div className="global-news-orbital-ring global-news-orbital-ring-a" />
              <div className="global-news-orbital-ring global-news-orbital-ring-b" />
              <div className="global-news-globe-wrapper">
                <svg ref={svgRef} className="global-news-globe-map" viewBox="0 0 1000 1000" aria-label="真实世界地图地球" />
              </div>
              {globeError ? <div className="global-news-globe-error">{globeError}</div> : null}
            </div>
          </div>
        </section>
        <aside className="global-news-side-panel">
          <div className="global-news-panel-header">
            <div>
              <p className="global-news-section-label">Country Feed</p>
              <div className="global-news-title-row">
                <h2>{feed.name}</h2>
                <button className="global-news-secondary-button" type="button" onClick={() => setSelectedCountryName(null)} hidden={!selectedCountryName}>返回全球</button>
              </div>
            </div>
            <span className="global-news-count-badge">{feed.news.length} 条资讯</span>
          </div>
          <section className="global-news-summary-card"><p className="global-news-summary-label">国家概览</p><p>{feed.summary}</p></section>
          <section className="global-news-tags">{feed.tags.map((tag) => <span key={tag} className="global-news-tag">{tag}</span>)}</section>
          <section className="global-news-list-section">
            <div className="global-news-list-header"><h3>最新资讯</h3><p>当前为示例数据，可继续替换为真实新闻 API</p></div>
            <div className="global-news-list">
              {feed.news.map((item) => (
                <article key={`${item.source}-${item.time}-${item.title}`} className="global-news-card">
                  <div className="global-news-meta"><span>{item.source}</span><span>{item.time}</span></div>
